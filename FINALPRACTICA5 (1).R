...
boxplot(data2, col=c("lightblue","lightblue","lightblue","lightgreen","lightgreen","lightgreen"), main="GSE5583-Boxplots(log2)",las=2)
...
head(wt.mean)
head(ko.mean)
# ¿Cuál es la media más alta? (maximo valor de expresion)
...
plot(ko.mean~wt.mean, xlab="WT", ylab="KO", main="GSE5583-ScatterPlot",col="pink",xlim=c(0,limit),ylim=c(0,limit))
...
#y=bx+a
#abline(x,y)
abline(1,2,col="aquamarine2")
abline(h=2,col="violet")
abline(v=3,col="seagreen3")
...
hist(pvalue,col="steelblue3")
hist(-log10(pvalue))
# Hacemos un volcano plot. Aquí podemos meter la diferencia de medias y la significancia estadística

plot(diff.mean,-log(pvalue),main="GSE5583-Volcano", col="violet")
#si le quitas log-10 queda un poquin peor
...
abline(v=diff.mean_cutoff,col="tan2",lwd=3)
abline(h=-log10(pvalue_cutoff),col="olivedrab4",lwd=3)
...
plot(diff.mean,-log10(pvalue),main="GSE5583-Volcano(filtered)",col="wheat2")
points(diff.mean[filtered_combined],-log10(pvalue[filtered_combined]),col="indianred3")

# Ahora vamos a marcar los que estarían sobreexpresados (rojo) y reprimidos (azul). ¿Por qué parece que están al revés?

plot(diff.mean,-log10(pvalue),main="GSE5583-Volcano(filtered)",col="wheat2")
points(diff.mean[filtered_combined & diff.mean < 0],-log10(pvalue[filtered_combined & diff.mean < 0]),col="indianred3")
points(diff.mean[filtered_combined & diff.mean > 0],-log10(pvalue[filtered_combined & diff.mean > 0]),col="steelblue3")
